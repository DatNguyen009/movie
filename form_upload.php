<?php
    ...
    if ($masp == "") {
      ...
      header("location:/DAT/Skincare/Admin/Addproduct/");
    }
    if ($tensp == "") {
      $_SESSION['masp'] = "".$masp;
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
      ...
      if ((count($fileNames) > 4) || (count($fileNames) == 0)) {
          ...
          $add_product = "INSERT INTO `product`(`prod_id`, `prod_name`, `prod_cost`, `prod_img`, `prod_amount`, `prod_description`, `prod_saleoff`,`Color`,`prod_type`) VALUES ('','$tensp',$giasp,'$mang[0]',$soluong,'$mieuta',$khuyenmai,'$color','$prod_type')";
          $qr = mysqli_query($con, $add_product);
          for ($i=0; $i < count($mang) ; $i++) { 
             
              mysqli_query($con, "INSERT INTO product_image(`prod_img`, `img_one`) VALUE('$mang[0]','$mang[$i]')");
              $_SESSION['success'] = "Thêm sản phẩm thành công";
              header("location:/DAT/Skincare/Admin/ProductManager/");
          }
